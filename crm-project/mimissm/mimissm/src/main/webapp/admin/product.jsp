...
<script type="text/javascript">
    ...
    function ckClick() {
        ...
        //获取被选中复选框的数量
        var checkdeLength = $("input[name='ck']:checked").length
        //判断全选复选框是否选中
        if (fiveLength==checkdeLength){
...
<script type="text/javascript">
    ...
    function deleteBatch() {
        ...
        // 获取选中的对象,获取value值进行字符串拼接
        if (cks.length==0){
            alert("当前没有选中商品");
        }else{
            var str = "";
            var pid = "";
            if (confirm("当前选中条"+cks.length+"记录，是否删除")){
                //遍历id
                $.each(cks,function () {
                    pid = $(this).val();//取出每一个选中的id。
                    //进行非空判断
                    if (pid != null){
                        str += pid+",";//1,4,5
                    }
    ...
    function del(pid,page) {
        ...
        if (confirm("确定删除吗")){
            ...
            //发送ajax请求删除
            $.ajax({
                url:"${pageContdext.request.contextPath}/prod/delete.action",
