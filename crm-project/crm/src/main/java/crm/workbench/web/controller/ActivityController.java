...
public class ActivityController {
    ...
    public void exportAllActivitys(HttpServletResponse response) throws Exception{
        ...
        //根据wb对象生成excel文件
        /*OutputStream os=new FileOutputStream("D:\\测试下载路径\\activityList.xls");
        wb.write(os);
        //关闭资源
        os.close();
        wb.close();*/
        ...
        //设置响应头信息，浏览器接收到之后直接激活下载窗口，不会自动打开。
        response.addHeader("Content-Disposition","attachment;filename=mystudentList.xls");
        //2.获取输出流
        OutputStream out= response.getOutputStream();
        //3.读取excel文件（InputStream）,输出到浏览器
        /*InputStream is= new FileInputStream("D:\\测试下载路径\\activityList.xls");
    ...
    public @ResponseBody Object importActivity(MultipartFile activityFile,HttpSession session){
        ...
        try {
            ...
            //根据excel文件生成HSSFSheet对象，封装了一页的所有信息
            HSSFSheet sheet=wb.getSheetAt(0);//页的下标从0开始依次递增
            ...
            for (int i=1;i<=sheet.getLastRowNum();i++){//sheet.getLastRowNum()最后一行的下标
                row=sheet.getRow(i);//行的下标从0开始依次递增
                ...
                for(int j=0;j<row.getLastCellNum();j++) {//row.getLastCellNum()最后一列的下标+1
                    //根据row获取HSSFCell对象封装了一列的信息
                    cell=row.getCell(j);//列的下标从0开始依次递增
